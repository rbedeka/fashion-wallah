...
import FadeCarousel,{links as FadeCss} from '~/components/fadeCarousel/FadeCarousel';
import HorizontalCarousel ,{links as HorizontalCss} from '../components/hozizontalCarousel/HorizontalCarousel';
import RatingFeed from '../components/RatingFeed';
import { HOME_BANNER_DATA, TEST_COLLECTIONS, TEST_RATING_DATA } from '~/testData/ComponentTestingData';
import { COLLECTIONS_QUERY, CollectionItem, CollectionsGrid } from './collections._index';
    ...
    <div className="home">
      ...
      {HOME_BANNER_DATA.map((item, index) => 
          <div key={item.id} className='m-0 p-0'>
              <div className={"flex justify-center items-center m-0  " + item.class}>
                <img src={item.image.url} key={item.image.id} className='rounded-md m-0 banner_image' />
              </div>
          </div>
        )}
        </FadeCarousel>
      ...
      <div className='w-full off_white_white'>
        <div className='text-xl text-center p-5'>
          FEATURED COLLECTION
        </div>
        <HorizontalCarousel slides_to_show={1} slides_to_show_resp={[1,1,1]} >
            <div className='w-full text-center text-xl p-5'>
      ...
      <div className='trans_taupe pb-20'>
        <ReviewsNRating count={[1407,23123]} label={['reviews','happy customers']} reviews = {[]}/>
      ...
      <div className='taupe-dark p-3'>
        ...
        <HorizontalCarousel slides_to_show={4} slides_to_show_resp={[4,3,1]} arrows={false}>
          {
          TEST_COLLECTIONS.map((item,index)=>(
              <CollectionItem collection={item} index={index} />
          ))
                ...
                >
                  ...
                  <div className='flex flex-col justify-center items-center'>
                    ...
                    <p className='text-sm text-extralight'>
                      <Money data={product.priceRange.minVariantPrice} className='text-extralight'/> 
                    </p>
                  </div>
                </Link>
              ))}
...
function ReviewsNRating({count,label,reviews}){
  return (
    <div className='text-center flex row_to_col justify-center items-center text-transparent bg-clip-text bg-gradient-to-b from-indigo-700 to-indigo-950 mb-15'>
      ...
      <div title='' className='flex flex-col justify-center items-center mx-20 my-10'>
